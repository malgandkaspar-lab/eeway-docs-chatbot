...
// Same keep-alive rationale as OLLAMA_KEEP_ALIVE in lib/llm.js - the
// embedder unloading between requests adds a multi-second reload tax to
// every search and question-cache lookup.
...
async function embed(text, { isQuery = false } = {}) {
  ...
  const res = await fetch(`${OLLAMA_BASE_URL}/api/embeddings`, {
    ...
    body: JSON.stringify({ model: EMBED_MODEL, prompt: prefix + text, keep_alive: EMBED_KEEP_ALIVE }),
  });
  if (!res.ok) {
